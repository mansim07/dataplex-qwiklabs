...
resource "null_resource" "setup_code" {
  provisioner "local-exec" {
    command = <<-EOT
      ...
      sed -i s/_region_/${var.location}/g code/merchant-source-configs/dq_merchant_data_product.yaml
      sed -i s/_region_/${var.location}/g code/merchant-source-configs/dq_merchant_gcs_data.yaml
      sed -i s/_region_/${var.location}/g code/merchant-source-configs/dq_merchant_gcs_data.yaml
      ...
      sed -i s/_region_/${var.location}/g code/transactions-source-configs/dq_transactions_data_product.yaml
      sed -i s/_region_/${var.location}/g code/transactions-source-configs/dq_transactions_gcs_data.yaml
      sed -i s/_region_/${var.location}/g code/transactions-consumer-configs/dq_cc_analytics_data_product.yaml
