  ...
  config {
    ...
    software_config {
      ...
      env_variables = {
        ...
        AIRFLOW_VAR_TRANSACTIONS_DPLX_REGION = "${var.location}",
...
resource "null_resource" "dag_setup" {
  provisioner "local-exec" {
    command = <<-EOT
    export airflow_dag_folder=$(gcloud composer environments describe ${var.project_id}-composer --project=${var.project_id} --location=${var.location} | grep dagGcsPrefix | awk  '{print $2}')
    export airflow_data_folder=$(gcloud composer environments describe ${var.project_id}-composer --project=${var.project_id} --location=${var.location} | grep dagGcsPrefix | awk  '{print $2}' | sed -e 's/dags/data/')
    gsutil mv gs://${local._dataplex_process_bucket_name}/composer/dags/* $airflow_dag_folder
    gsutil mv gs://${local._dataplex_process_bucket_name}/composer/customer_data_products $airflow_data_folder/
    gsutil mv gs://${local._dataplex_process_bucket_name}/composer/merchant_data_products $airflow_data_folder/
    gsutil mv gs://${local._dataplex_process_bucket_name}/composer/transactions_data_products $airflow_data_folder/
    EOT
    }
    depends_on = [
